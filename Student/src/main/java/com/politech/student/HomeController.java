...
public class HomeController {
	 ...
	 */
	@RequestMapping(value = "/", method = RequestMethod.GET) // value="/"<-- 에는 package의 프로젝트의 이름인 Student가 기본적으로 들어간다.
...
//		ArrayList<People> studentList = dbCommon.selectArrayList(new People());
	...
	public String deleteData(@RequestParam("idx") int idx, @RequestParam("name") String name, @RequestParam("address") String address, 
			@RequestParam("birthday_month") String birthdayMonth, @RequestParam("birthday_day") String birthdayDay, @RequestParam("favoriteColor") String favoriteColor) {
			DBCommon<Student> dbCommon = new DBCommon<Student>("c:\\tomcat\\student190527.sqlite", "student");
			dbCommon.deleteData(new Student(name, address, birthdayMonth + "/" + birthdayDay, favoriteColor), idx);
			return "delete";
		}
